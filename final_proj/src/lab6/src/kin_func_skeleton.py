...
def skew_3d(omega):
    ...
    wy = omega[1]
    wz = omega[2]
    omega_hat = np.array([[0,-wz,wy],[wz,0,-wx], [-wy,wx,0]])
...
def rotation_3d(omega, theta):
    ...
    #YOUR CODE HERE
    rot = LA.expm(np.dot(skew_3d(omega),theta))
    return rot
...
def hat_2d(xi):
    ...
    vy = xi[1]
    w = xi[2]
    xi_hat = np.array([[0,-w,vx],[w,0,vy],[0,0,0]])
    return xi_hat
...
def homog_2d(xi, theta):
    ...
    #YOUR CODE HERE
    g = LA.expm(np.dot(hat_2d(xi),theta))
    return g
...
def homog_3d(xi, theta):
    ...
    #YOUR CODE HERE
    g = LA.expm(np.dot(hat_3d(xi),theta))
