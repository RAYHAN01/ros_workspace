...
def find_v(omega, theta, trans):
    ...
    """    
    #YOUR CODE HERE
    omega1 = omega.reshape((3,1))
    A = np.dot((np.eye(3)-kfs.rotation_3d(omega,theta)),kfs.skew_3d(omega))+np.dot(omega1,omega1.T)*theta
    p = trans.reshape((3,1))
    v = np.dot(LA.inv(A),p)
    return v
